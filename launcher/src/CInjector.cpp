...
HMODULE WINAPI GetRemoteModuleHandleW(HANDLE hProcess, LPCWSTR lpModuleName)
{
	...
	for (size_t i = 0; moduleName[i] != '\0'; ++i)
	{
		if (moduleName[i] >= 'A' && moduleName[i] <= 'Z')
			moduleName[i] += 0x20; // 0x20 is the difference between uppercase and lowercase
	}
	...
	for (DWORD i = 0; i <= NumModules; ++i)
	{
		...
		std::wstring _moduleName = File::GetRemoteModuleName(hProcess, ModuleArray[i]);

		/* Convert ModuleNameBuffer to all lowercase so the comparison isn't case sensitive */
		for (size_t j = 0; _moduleName[j] != '\0'; ++j)
		{
			if (_moduleName[j] >= 'A' && _moduleName[j] <= 'Z')
				_moduleName[j] += 0x20; // 0x20 is the difference between uppercase and lowercase
		}

		/* Does the name match? */
		if (wcsstr(_moduleName.c_str(), moduleName.c_str()) != NULL)
		{
...
FARPROC WINAPI GetRemoteProcAddressW(HANDLE hProcess, HMODULE hModule, LPCWSTR lpProcName, UINT Ordinal, BOOL UseOrdinal)
{
	BOOL Is64Bit = FALSE;
	...
	if (UseOrdinal)
	{
			...
			ExportFunctionTable[FunctionTableIndex] <= ExportDirectory.VirtualAddress + ExportDirectory.Size)
		{
			...
			std::wstring RealModuleName, RealFunctionId;
			RealModuleName = TempForwardString.substr(0, Dot - 1);
			RealFunctionId = TempForwardString.substr(Dot + 1, std::string::npos);

			HMODULE RealModule = GetRemoteModuleHandleW(hProcess, RealModuleName.c_str());
			...
			if (RealFunctionId.at(0) == '#') // Exported by ordinal
			{
				...
				TempReturn = GetRemoteProcAddressW(hProcess, RealModule, NULL, RealOrdinal, TRUE);
			}
			else // Exported by name
			{
				/* Recursively call this function to get return value */
				TempReturn = GetRemoteProcAddressW(hProcess, RealModule, RealFunctionId.c_str(), 0, FALSE);
			}
	...
	for (DWORD i = 0; i < ExportTable.NumberOfNames; ++i) {
		...
		/* Does the name match? */
		if (TempFunctionName.find(lpProcName) != std::wstring::npos)
		{
			/* NOTE:
				...
				ExportFunctionTable[ExportOrdinalTable[i]] <= ExportDirectory.VirtualAddress + ExportDirectory.Size)
			{
				...
				RealFunctionId = TempForwardString.substr(Dot + 1, std::wstring::npos);
