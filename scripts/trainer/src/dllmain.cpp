...
typedef void*(*GetGlobalPtrFunc)(uint32_t);
GetGlobalPtrFunc GetGlobalPtr = nullptr;
...
void ChangePlayerModel(const char* modelName)
{
	...
	Native::Invoke<void, uint32_t, uint32_t, bool>(N::SET_PLAYER_MODEL, 0, model, true);
	Native::Invoke<void, uint32_t>(N::SET_MODEL_AS_NO_LONGER_NEEDED, model);
}
...
extern "C" {
	DLL_EXPORT void Init(GetNativeAddressFunc getAddress, GetGlobalPtrFunc getGlobal)
	{
